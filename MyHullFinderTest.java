...
/**
 * This class can be used to test the functionality of you convex hull implementation.
 * You will find s few examples to guide you through the syntax of writing test cases.
 * Each test case uses its own HullFinder instance, to ensure that the test cases are independent 
 * of each other. All of these given examples should also pass once you've implemented your hull finder.
 ...
 *
 *
 * TODO: Give an overview of your tests and a brief explanation of why you believe you have covered all possible cases.
 */
...
public class MyHullFinderTest {
	...
	*/
	...
	/**
	 * An example of testing the hull when incremental add is used. Read the inline comments
	 * to understand the set-up.
	 * If you attempt angularGrahamScan, a similar method could be followed to test it.
	 */
	@Test
	public void testAddIncremental() {
		MyHullFinder hull = new MyHullFinder(_imagePath);
		// Create a list of coordinate pairs of points (100, 200),  (300, 300) , (100, 100) and (138, 188)
		List<Integer> coordinates = Arrays.asList(100, 200, 300, 300, 100, 100, 138, 188);
		
		// Convert from coordinates to a list of HullPoints using a helper method
		...
		// Form a list of HullPoints you know would form the correct hull out of your test points.
		List<HullPoint> expectedPoints = getPoints(Arrays.asList(100,200,300,300,100,100));	
		
		// Using the hull's iterator, create a list of the actual points in the hull
		...
		/*
		 * This assertion checks that the set of expected points contains all of the actual points
		 * and vice-versa. Checking this both ways ensures that the lists in fact contain the same 
		 * set of points. Note that two HullPoints are equal if both coordinates of the points are equal.
		 */
		assertTrue(expectedPoints.containsAll(actualPoints) && actualPoints.containsAll(expectedPoints));	
		
		// another way to check the same thing is to use two assertThat statements
	 ...
	 * Don't forget to test any helper methods you write.
	 */
	...
	/**
	 * Below are some helper methods you might find useful for testing.
	 * Feel free to add your own
	 */
	
	/**
	 * Given a list of coordinate pairs as (x1, y1, x2, y2, ...),
	 * creates and returns a list of HullPoints with those coordinates
	 * @param List<Integer> of coordinates to convert to points
